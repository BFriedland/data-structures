...
class test_ShortestPathsGraph(unittest.TestCase):

    def test_shortest_paths_algorithms(self):

        graph_zero = shortest_paths.ShortestPathsGraph()

        for each_integer in range(0, 10):
            graph_zero.add_node(each_integer)

        for each_index in range(1, len(graph_zero.node_list)):
            graph_zero.add_weighted_edge((each_index - 1), each_index, 1)

        # Tie the graph chain together at the ends, making a circle:
        graph_zero.add_weighted_edge(0, (len(graph_zero.node_list) - 1), 20)

        result_zero = graph_zero.dijkstra_algorithm(0, 5)
        result_zero_star = graph_zero.a_star_algorithm(0, 5)
        assert result_zero == (5, [0, 1, 2, 3, 4, 5])
        assert result_zero_star == (5, [0, 1, 2, 3, 4, 5])

        result_one = graph_zero.dijkstra_algorithm(9, 5)
        result_one_star = graph_zero.a_star_algorithm(9, 5)
        assert result_one == (4, [9, 8, 7, 6, 5])
        assert result_one_star == (4, [9, 8, 7, 6, 5])

        graph_one = shortest_paths.ShortestPathsGraph()

        graph_one.add_weighted_edge("alpha", "beta", 1)
        # Testing the decision making capability of this algorithm can be
        # as easy as flipping this large number to a small one:
        ...
        result_two = graph_one.dijkstra_algorithm("alpha", "delta")
        result_two_star = graph_one.a_star_algorithm("alpha", "delta")
        assert result_two == (647172, ['alpha',
                                       'loopa',
                                       'doopa',
                                       'hoopa',
                                       'gamma',
                                       'delta'])
        assert result_two_star == (647172, ['alpha',
                                            'loopa',
                                            'doopa',
                                            'hoopa',
                                            'gamma',
        ...
        result_three = graph_one.dijkstra_algorithm("alpha", "delta")
        result_three_star = graph_one.a_star_algorithm("alpha", "delta")
        assert result_three == (3, ['alpha', 'beta', 'gamma', 'delta'])
        assert result_three_star == (3, ['alpha', 'beta', 'gamma', 'delta'])

        # Separated:
        graph_one.add_node("omicron")

        result_four = graph_one.dijkstra_algorithm("alpha", "omicron")
        result_four_star = graph_one.a_star_algorithm("alpha", "omicron")
        assert result_four == None
        assert result_four == None
    ...
    def test_both_traversals(self):
        ...
        # including ones with isolated vertices and separate webs.

        for each_pass in range(0, 100):

            random_graph = shortest_paths.ShortestPathsGraph()
            random_node_count = random.randint(10, 100)
            # -1 to allow for single-node ShortestPathsGraphs
    ...
    # Testing the rest of ShortestPathsGraph:

    def setUp(self):

        # Note: This test method tests add_edge(), add_node() and
        # the ShortestPathsGraph constructor.

        self.empty_graph = shortest_paths.ShortestPathsGraph()

        self.linear_graph = shortest_paths.ShortestPathsGraph()
        for each_integer in range(0, 10):
        ...
        self.circular_graph = shortest_paths.ShortestPathsGraph()
        for each_integer in range(0, 10):
    ...
    def test_del_edge(self):
        ...
        # Make sure the Edges are not in the ShortestPathsGraph:
    ...
    def test_neighbors(self):
        ...
        # Additional required test case: Node-type not in ShortestPathsGraph
        with self.assertRaises(Exception):
