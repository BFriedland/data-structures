...
class test_WeightedGraph(unittest.TestCase):
    ...
    # Testing the rest of WeightedGraph:
    ...
    def test_del_edge(self):
        ...
        # Make sure the Edges are not in the WeightedGraph:
    ...
    def test_neighbors(self):
        ...
        # "raises an error if n is not in g"
        # Additional required test case: Node-type not in WeightedGraph
        with self.assertRaises(Exception):
    ...
    def test_add_weighted_edge_and_return_weighting(self):

        self.setUp()

        with self.assertRaises(Exception):
            self.empty_graph.add_weighted_edge()
            self.empty_graph.add_weighted_edge(2)
            self.empty_graph.add_weighted_edge(3, 4)
            self.empty_graph.add_weighted_edge("Strings", "are", "fun")

        # Various ordinary Edges constructed with different argument ordering:
        self.circular_graph.add_weighted_edge(1, 2, 5)
        assert self.circular_graph.return_weighting(1, 2) == 5
        self.circular_graph.add_weighted_edge(6, 3, 4)
        assert self.circular_graph.return_weighting(3, 6) == 4

        # Negative nodes being constructed through add_edge:
        self.circular_graph.add_weighted_edge(-1, 0, 5)
        assert self.circular_graph.return_weighting(-1, 0) == 5

        # Flippin' arguments! Strings too.
        self.circular_graph.add_weighted_edge(-6, "stringzzz", -4)
        assert self.circular_graph.return_weighting("stringzzz", -6) == -4
